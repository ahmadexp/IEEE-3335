...
- The word *Architecture* is an overloaded term much favored by marketing organizations. In the
- TimeCard context, there are two important complementary technical definitions.
- At the *system* level (a level above both hardware and software implementation details), the
- *Architectural Principles* are a set of black-box qualities or properties et al that the system in
- question must exhibit or possess.
- At the *hardware* and *software* level, the *Architectural Constraints* are a set of often interacting
- constraints and driving requirements that will be analyzed in context to find the best way to
- implement the Architectural Principles using what's available in practice.
...
- Timestamps **shall** support a mathematical *distance metric*, so algebraic differences in timestamp
...
- (1588-2019) Mixed Multicast Unicast Operations "Hybrid" mode. Hybrid mode may work with
- PTPv2 (1588-2008) as well, but this is not required.
- The TimeCard standard **shall** itself be structured such that widely used legacy TimeCard units
- automatically conform, allowing direct replacement of old by improved new TimeCards without
- requiring hardware changes in the host system being provided with time reference signals.
- Application and operating system software changes are acceptable, being an expected part of the
- upgrade project. << Needs a better description of acceptable changes. >>
- A TimeCard **shall** have at least one time-reference input connector (perhaps fed from a local
- primary reference source such as a Cesium-Beam Primary Reference Standard or a non-standard
- Radio Clock of some kind.
...
- generated and when full performance is achieved. Both the maximum time to first usable signal
- and the maximum time to full performance **shall** be specified, measured from application of
- power.

- The maximum time required to achieve specified performance after resynch due to an abrupt
- change of a pre-specified kind, including step discontinuities in incoming time references such as
- during a fail-over event, **shall** be specified.
- Bridging step discontinuities implies the ability to command a time step before re-closing the
- PLL control loops, so we don't have to wait for hours while the PLL slowly slews, or just gives
- up because the needed jump is too large. The capability to change the PLL loop time constant
- without resetting the loop-filter integrator **shall** be provided. << Modeled on the Datum model
- 1050 Disciplined Oscillator, which switches the feedback resistor value to change the time
- constant, leaving the charge in the capacitor unchanged. >>
- Required stability and graceful autonomous handling of overloads and subsequent recovery
- when overloads subside and cease **shall** be provided. << Needs more precise wording. >>
- Support for traceability verification analysis [G8260] of systems utilizing one or more TimeCard
- units **shall** be provided. This implies that all data needed to perform traceability analysis is
- publicly available. << Traceability is more loosely defined in IEEE 1588v2.1 and so in P3335
