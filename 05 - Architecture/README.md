...
- At least one timing providing interface.
...
Providing interfaces distribute time, phase, frequency, or timestamp information from the TimeCard to a host system or downstream consumer. Providing interfaces are specified in Clause 7.
...
If PCIe PTM is implemented, the implementation shall document the PTM capability, the measurement point represented by PTM timestamps, and any correction terms needed to relate PTM time to other providing interfaces.
...
An implementation may include optional features or vendor-specific extensions. Optional features and extensions shall not prevent baseline operation of the documented conforming interfaces.

Vendor-specific extensions shall be documented sufficiently to allow an operator to distinguish standardized P3335 behavior from implementation-specific behavior. Reserved values, register fields, message types, and identifiers shall not be repurposed in a way that conflicts with this standard.
...
- Traceability information sufficient for operational analysis, or an explicit statement that traceability is not supported.
